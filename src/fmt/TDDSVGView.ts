...
// Erweitere das jQuery-Interface in TypeScript
declare global {
  interface JQuery {
    svg(method?: string): any
  }
}
...
class TDDSVGView {
  ...
  constructor() {
    ...
    this.bg = this.svg.rect(0, 0, fullwidth, fullheight, {
      fill: '#ffffff',
      stroke: '#000000',
      strokeWidth: 0,
    })

    var fillpat = this.svg.pattern('fillpat', 0, 0, 4, 4, {
  ...
  conform(draft) {
    ...
    if (
      ...
      (this.threading.length > 0 &&
        this.threading[0].holes.length != draft.holes()) ||
  ...
  conform_turning(draft) {
    if (this.show_turning) {
      ...
      if (this.repeats != undefined) {
        for (
          var y = draft.picks() - 1;
          y > draft.picks() - this.start_pick;
          y--
        ) {
          if ((draft.threading[x] == 'S') == (draft.turning[y][x] == '/')) {
            tablet_position[x] =
              (tablet_position[x] + draft.holes() - 1) % draft.holes()
          } else {
        ...
        while (cell >= 0) {
          for (
            var y = draft.picks() - this.start_pick;
            y >= draft.picks() - this.end_pick;
            y--
          ) {
            //alert("y: " + y + " cell: " + cell);
            ...
            for (var x = 0; x < draft.tablets(); x++) {
              ...
              if ((draft.threading[x] == 'S') == (draft.turning[y][x] == '/')) {
                ...
                fg = draft.threadColour(
                  x,
                  (tablet_position[x] + 1) % draft.holes()
                )
  ...
  create_cell(
    ...
    var g = this.svg.group(group, {
      transform: 'translate(' + X_coord + ',' + Y_coord + ') ' + 'rotate(45)',
    })

    var ell = this.svg.ellipse(g, 0, 0, cellheight / 2, cellheight / 4, {
      fill: '#000000',
      stroke: '#000000',
      strokeWidth: 1,
    })
  ...
  svg_coord_to_pick(y: number, draft: TDDDraft) {
    ...
    if (
      !this.show_turning ||
      y >= turning_start_y + (cellborder + cellheight) * draft.picks()
    ) {
      return -1
    } else {
      return (
        draft.picks() - (y - turning_start_y) / (cellborder + cellheight) - 1
      )
    }
  }

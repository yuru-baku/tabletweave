...
(function ($) {
  ...
  $.extend(SVGManager.prototype, {
    ...
    _attachSVG: function (container, settings) {
      let svg = container.namespaceURI === this.svgNS ? container : null;
      let container = svg ? null : container;
    ...
    _afterLoad: function (container, svg, settings) {
      let settings = settings || this._settings[container.id];
      this._settings[container ? container.id : ''] = null;
      let wrapper = new this._wrapperClass(svg, container);
  ...
  function SVGWrapper(svg, container) {
    ...
    for (let i = 0; i < $.svg._extensions.length; i++) {
      let extension = $.svg._extensions[i];
  ...
  $.extend(SVGWrapper.prototype, {
    ...
    configure: function (node, settings, clear) {
      ...
      if (clear) {
        for (let i = node.attributes.length - 1; i >= 0; i--) {
          ...
          if (
            !(
              attr.nodeName === 'onload' ||
              attr.nodeName === 'version' ||
      ...
      for (let attrName in settings) {
        node.setAttribute(
          $.svg._attrNames[attrName] || attrName,
          settings[attrName]
        );
      }
      return this;
    ...
    _args: function (values, names, optSettings) {
      ...
      if (
        values[0] != null &&
        !(typeof values[0] === 'object' && values[0].nodeName)
      ) {
        args['parent'] = null;
        offset = 1;
      }
    ...
    title: function (parent, text, settings) {
      let args = this._args(arguments, ['text']);
      let node = this._makeNode(args.parent, 'title', args.settings || {});
    ...
    describe: function (parent, text, settings) {
      let args = this._args(arguments, ['text']);
      let node = this._makeNode(args.parent, 'desc', args.settings || {});
      node.appendChild(this._svg.ownerDocument.createTextNode(args.text));
      return node;
    },
    ...
    defs: function (parent, id, settings) {
      let args = this._args(arguments, ['id'], ['id']);
      return this._makeNode(
        args.parent,
        'defs',
        $.extend(args.id ? { id: args.id } : {}, args.settings || {})
      );
    },
    ...
    symbol: function (parent, id, x1, y1, width, height, settings) {
      let args = this._args(arguments, ['id', 'x1', 'y1', 'width', 'height']);
      return this._makeNode(
        args.parent,
        'symbol',
        $.extend(
    ...
    style: function (parent, styles, settings) {
      let args = this._args(arguments, ['styles']);
    ...
    script: function (parent, script, type, settings) {
      let args = this._args(arguments, ['script', 'type'], ['type']);
      let node = this._makeNode(
        args.parent,
        'script',
        $.extend({ type: args.type || 'text/javascript' }, args.settings || {})
      );
    ...
    _gradient: function (parent, name, settings, stops) {
      let node = this._makeNode(parent, name, settings);
    ...
    clipPath: function (parent, id, units, settings) {
      let args = this._args(arguments, ['id', 'units']);
      args.units = args.units || 'userSpaceOnUse';
      return this._makeNode(
        args.parent,
        'clipPath',
        $.extend(
          { id: args.id, clipPathUnits: args.units },
          args.settings || {}
        )
      );
    },
    ...
    mask: function (parent, id, x, y, width, height, settings) {
      let args = this._args(arguments, ['id', 'x', 'y', 'width', 'height']);
      return this._makeNode(
        args.parent,
        'mask',
        $.extend(
    ...
    use: function (parent, x, y, width, height, ref, settings) {
      let args = this._args(arguments, ['x', 'y', 'width', 'height', 'ref']);
    ...
    link: function (parent, ref, settings) {
      let args = this._args(arguments, ['ref']);
      let node = this._makeNode(args.parent, 'a', args.settings);
      node.setAttributeNS($.svg.xlinkNS, 'href', args.ref);
      return node;
    },
    ...
    path: function (parent, path, settings) {
      let args = this._args(arguments, ['path']);
      return this._makeNode(
        args.parent,
        'path',
        $.extend(
    ...
    circle: function (parent, cx, cy, r, settings) {
      let args = this._args(arguments, ['cx', 'cy', 'r']);
      return this._makeNode(
        args.parent,
        'circle',
        $.extend({ cx: args.cx, cy: args.cy, r: args.r }, args.settings || {})
      );
    },
    ...
    ellipse: function (parent, cx, cy, rx, ry, settings) {
      let args = this._args(arguments, ['cx', 'cy', 'rx', 'ry']);
      return this._makeNode(
        args.parent,
        'ellipse',
        $.extend(
          { cx: args.cx, cy: args.cy, rx: args.rx, ry: args.ry },
          args.settings || {}
        )
      );
    },
    ...
    line: function (parent, x1, y1, x2, y2, settings) {
      let args = this._args(arguments, ['x1', 'y1', 'x2', 'y2']);
      return this._makeNode(
        args.parent,
        'line',
        $.extend(
    ...
    polyline: function (parent, points, settings) {
      let args = this._args(arguments, ['points']);
    ...
    text: function (parent, x, y, value, settings) {
      let args = this._args(arguments, ['x', 'y', 'value']);
    ...
    textpath: function (parent, path, value, settings) {
      let args = this._args(arguments, ['path', 'value']);
      let node = this._text(
        args.parent,
        'textPath',
        args.value,
    ...
    _text: function (parent, name, value, settings) {
      ...
      if (typeof value === 'string') {
        ...
        for (let i = 0; i < value._parts.length; i++) {
          let part = value._parts[i];
          if (part[0] === 'tspan') {
            let child = this._makeNode(node, part[0], part[2]);
            ...
            let set = $.extend({}, part[2]);
            set.href = null;
            let child = this._makeNode(node, part[0], set);
    ...
    other: function (parent, name, settings) {
      let args = this._args(arguments, ['name']);
    ...
    _makeNode: function (parent, name, settings) {
      ...
      let node = this._svg.ownerDocument.createElementNS($.svg.svgNS, name);
      for (let name in settings) {
    ...
    add: function (parent, node) {
      let args = this._args(
        arguments.length === 1 ? [null, parent] : arguments,
        ['node']
      );
      let svg = this;
      ...
      try {
        ...
        args.node.each(function () {
          let child = svg._cloneAsSVG(this);
          if (child) {
    ...
    _cloneAsSVG: function (node) {
      ...
      if (node.nodeType === 1) {
          ...
          if (attr.nodeName !== 'xmlns' && attr.nodeValue) {
            if (attr.prefix === 'xlink') {
              newNode.setAttributeNS(
                $.svg.xlinkNS,
                attr.localName || attr.baseName,
                attr.nodeValue
              );
            } else {
              newNode.setAttribute(
                this._checkName(attr.nodeName),
                attr.nodeValue
              );
            }
          }
        ...
        for (let i = 0; i < node.childNodes.length; i++) {
          let child = this._cloneAsSVG(node.childNodes[i]);
          if (child) {
    ...
    _checkName: function (name) {
      name =
        name.substring(0, 1) >= 'A' && name.substring(0, 1) <= 'Z'
          ? name.toLowerCase()
          : name;
    ...
    load: function (url, settings) {
      ...
      let reportError = function (message) {
      ...
      // Load the SVG DOM
      let loadSVG = function (data) {
        if (!data) {
          return;
        }
        if (data.documentElement.nodeName !== 'svg') {
          let errors = data.getElementsByTagName('parsererror');
          let messages = errors.length
            ? errors[0].getElementsByTagName('div')
            : []; // Safari
          reportError(
            !errors.length
              ? '???'
        ...
        let parent = settings.parent ? $(settings.parent)[0] : wrapper._svg;
        let attrs = {};
        for (let i = 0; i < data.documentElement.attributes.length; i++) {
          let attr = data.documentElement.attributes.item(i);
          if (
            !(
              attr.nodeName === 'version' ||
              attr.nodeName.substring(0, 5) === 'xmlns'
        ...
        let nodes = data.documentElement.childNodes;
        ...
        if (node.attributes) {
          for (let i = 0; i < node.attributes.length; i++) {
            ...
            if (
              ...
              svgDoc +=
                ' ' +
                (attr.namespaceURI === $.svg.xlinkNS ? 'xlink:' : '') +
                attr.nodeName +
                '="' +
                attr.nodeValue +
                '"';
  ...
  /** Helper to generate an SVG path.
    ...
    @module SVGPath
    @example let path = root.createPath();
  ...
  /** Helper to generate an SVG text object.
    ...
    @module SVGText
    @example let text = root.createText();
  ...
  $.fn.svg = function (options) {
    ...
    if (typeof options === 'string' && options === 'get') {
      return $.svg['_' + options + 'SVG'].apply(
        $.svg,

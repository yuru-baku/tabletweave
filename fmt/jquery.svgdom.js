...
;(function ($) {
  ...
  function setClassNames(elem, classes) {
    elem.className
      ? (elem.className.baseVal = classes)
  ...
  $.fn.toggleClass = (function (origToggleClass) {
    return function (classNames, state) {
      if ($.isFunction(classNames)) {
        return this.each(function (i) {
          $(this).toggleClass(
            classNames.call(this, i, getClassNames(this), state),
            state
          )
        })
      }
          ...
          if (name === 'transform') {
            for (var i = 0; i < val.numberOfItems; i++) {
              ...
              switch (item.type) {
                ...
                case 2:
                  value +=
                    ' translate(' + item.matrix.e + ',' + item.matrix.f + ')'
                  break
                case 3:
  ...
  if ($.cssProps) {
    $.css = (function (origCSS) {
      return function (elem, name, numeric, extra) {
        var value = name.match(/^svg.*/)
          ? $(elem).attr($.cssProps[name] || name)
          : ''
